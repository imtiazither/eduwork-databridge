...
The orchestration layer includes Dagster asset definitions for external inspection plus a local-first asset engine used by tests and the API.
...
Dagster definitions expose raw snapshot, profile, validated, and mart assets plus a daily job/schedule. Deployment of a Dagster daemon or managed service requires environment-specific validation.
