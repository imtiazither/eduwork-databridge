...
def verify(directory: Path, output: Path) -> None:
    wheels = sorted(directory.glob("eduwork_databridge-0.14.0-*.whl"))
    sdists = sorted(directory.glob("eduwork_databridge-0.14.0.tar.gz"))
    if len(wheels) != 1 or len(sdists) != 1:
        raise SystemExit("Expected one v0.14.0 wheel and one source distribution")
    ...
    with zipfile.ZipFile(wheel) as archive:
        ...
        required_wheel = {
            ...
            "eduwork_databridge-0.14.0.dist-info/METADATA",
            "eduwork_databridge-0.14.0.dist-info/licenses/LICENSE",
        }
        if not required_wheel <= names:
            raise SystemExit("Wheel is missing required files")
        metadata = archive.read("eduwork_databridge-0.14.0.dist-info/METADATA").decode()
        if "Version: 0.14.0" not in metadata or "Requires-Python: <3.14,>=3.12" not in metadata:
            raise SystemExit("Wheel metadata version or Python range is invalid")
    with tarfile.open(sdist, "r:gz") as archive:
        names = set(archive.getnames())
        prefix = "eduwork_databridge-0.14.0/"
        required_sdist = {
